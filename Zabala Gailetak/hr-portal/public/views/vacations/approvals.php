 ...
 * 
 * Shows pending vacation requests for manager approval
 */

declare(strict_types=1);

$pageTitle = 'Opor Onarpena / Aprobaciones de Vacaciones';
...
<div class="container">
    ...
    <div id="pending-requests" class="section-card" style="display: none;">
        <h2>Onarpen Zain / Pendientes de Aprobación</h2>
...
<div id="action-modal" class="modal" style="display: none;">
    <div class="modal-content">
        ...
        <div class="form-group">
            <label for="action-notes">Oharrak / Notas</label>
...
function openApproveModal(requestId, employeeName) {
    ...
    document.getElementById('modal-title').textContent = 'Opor eskaera onartu / Aprobar solicitud';
    document.getElementById('modal-body').innerHTML = `
        <p>Ziur zaude <strong>${employeeName}</strong>-ren eskaera onartu nahi duzula?</p>
        <p>¿Estás seguro de que quieres aprobar la solicitud de <strong>${employeeName}</strong>?</p>
    `;
...
function openRejectModal(requestId, employeeName) {
    ...
    document.getElementById('modal-title').textContent = 'Opor eskaera ukatu / Rechazar solicitud';
    document.getElementById('modal-body').innerHTML = `
        <p>Ziur zaude <strong>${employeeName}</strong>-ren eskaera ukatu nahi duzula?</p>
        <p>¿Estás seguro de que quieres rechazar la solicitud de <strong>${employeeName}</strong>?</p>
...
async function confirmAction() {
    ...
    try {
        ...
        if (currentAction === 'approve') {
            await apiCall(`/vacations/requests/${currentRequestId}/approve-manager`, {
                method: 'POST',
                body: JSON.stringify({ notes: notes || null })
            });
            showSuccess('✓ Eskaera onartua / Solicitud aprobada');
...
async function loadRequests() {
    try {
        ...
        const data = await apiCall('/vacations/pending/manager');
