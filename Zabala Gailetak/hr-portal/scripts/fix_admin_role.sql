...
SELECT id, email, role, account_locked, mfa_enabled, created_at
FROM users
...
UPDATE users 
SET role = 'ADMIN'
WHERE email = 'admin@zabalagailetak.com' AND role = 'admin';
...
-- 4. Verificar cambios
SELECT id, email, role, account_locked, mfa_enabled, failed_login_attempts
FROM users
WHERE email = 'admin@zabalagailetak.com';
...
UPDATE users SET role = 'JEFE_SECCION' WHERE LOWER(role) = 'jefe_seccion';
...
SELECT id, email, role, account_locked FROM users ORDER BY created_at;
